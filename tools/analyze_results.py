...
def analyze_results(results_dir="results_kor", output_dir="analyze_results"):
    ...
    for file in results_files:
        try:
            with open(file, "r", encoding="utf-8") as f:
                ...
                # 히트맵 데이터 수집 (평균 계산을 위해 리스트로 저장)
                key = (context_length, depth)
                if key not in heatmap_data:
                    heatmap_data[key] = []
                heatmap_data[key].append(data.get("score", 0))
    ...
    print(
        f"  총 테스트 시간: {sum(all_durations):.2f}초 ({sum(all_durations) / 60:.2f}분)"
    )
    ...
    for context_length in sorted(scores_by_context_length.keys()):
        ...
        print(
            f"  {context_length:6d} 토큰: {avg_score:5.2f}/10 ({len(scores)}개 테스트)"
        )
    ...
    summary = {
        ...
        "scores_by_context_length": {
            k: statistics.mean(v) for k, v in scores_by_context_length.items()
        },
...
def create_heatmap(heatmap_data, output_dir, model_name):
    ...
    Args:
        heatmap_data: (context_length, depth) -> [scores] 딕셔너리 (점수 리스트)
        output_dir: 출력 디렉토리
        model_name: 모델 이름
    """
    if not heatmap_data:
    ...
    for i, depth in enumerate(depths):
        for j, context_length in enumerate(context_lengths):
            if (context_length, depth) in heatmap_data:
                scores = heatmap_data[(context_length, depth)]
                matrix[i, j] = sum(scores) / len(scores)  # 평균 계산
    ...
    ax.set_title(
        f"Needle In Haystack 테스트 결과 히트맵\n모델: {model_name}",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def save_text_report(
    ...
    with open(report_file, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"  총 테스트 시간: {summary['total_duration']:.2f}초 ({summary['total_duration'] / 60:.2f}분)\n\n"
        )
